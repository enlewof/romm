...
def upgrade() -> None:
    ...
    with op.batch_alter_table("roms") as batch_op:
        add_column_if_not_exists(
            batch_op, sa.Column("p_name", sa.String(length=150), nullable=True)
        )
        add_column_if_not_exists(
            batch_op, sa.Column("url_cover", sa.Text(), nullable=True)
        )
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table("platforms") as batch_op:
        drop_column_if_exists(batch_op, "fs_slug")
    with op.batch_alter_table("roms") as batch_op:
        drop_column_if_exists(batch_op, "id")
        drop_column_if_exists(batch_op, "p_name")
        drop_column_if_exists(batch_op, "url_cover")
        batch_op.alter_column(
            "r_name",
