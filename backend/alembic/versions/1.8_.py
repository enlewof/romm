...
def upgrade() -> None:
    ...
    with op.batch_alter_table("platforms") as batch_op:
        ...
        drop_table_if_exists(op, "old_platforms")

    with op.batch_alter_table("roms") as batch_op:
        add_column_if_not_exists(
            op, "roms", sa.Column("p_name", sa.String(length=150), nullable=True)
        )
        add_column_if_not_exists(
            op, "roms", sa.Column("url_cover", sa.Text(), nullable=True)
        )
    ...
    with op.batch_alter_table("roms") as batch_op:
        ...
        batch_op.execute("INSERT INTO roms(\
                            ...
                            region, revision, tags, multi, files, url_cover \
                            FROM old_roms")
        drop_table_if_exists(op, "old_roms")
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table("platforms") as batch_op:
        drop_column_if_exists(op, "platforms", "fs_slug")
    with op.batch_alter_table("roms") as batch_op:
        drop_column_if_exists(op, "roms", "id")
        drop_column_if_exists(op, "roms", "p_name")
        drop_column_if_exists(op, "roms", "url_cover")
        batch_op.alter_column(
            "r_name",
